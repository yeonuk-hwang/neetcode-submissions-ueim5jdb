class Solution:
    def maxArea(self, heights: List[int]) -> int:
      ...
      # 그런데 둘이 같다면?
      # 같다면, left, right 중 다음 숫자가 더 큰 쪽으로 움직이는 것이 맞나? 아니 상관없다
      # 왜냐면 둘 중 어느 것을 움직여도, 현재의 area보다 커질 수 없다 왜냐하면 최대로 큰 min 값이 어차피 현재의 height이기 때문에
      # distance만 줄어들기에 값이 커질 수 없다, 따라서 둘 다 움직이거나 어느쪽을 움직여도 상관없다.
